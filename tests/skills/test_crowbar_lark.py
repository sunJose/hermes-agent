...
def test_default_output_redacts_ids_and_omits_content(monkeypatch):
    module = _load_module()
    monkeypatch.setenv("LARK_TENANT_ACCESS_TOKEN", "tenant-token-secret")

    def fake_request(method, url, headers, body=None):
        assert headers["Authorization"] == "Bearer tenant-token-secret"
        return {"code": 0, "data": {"items": [_message()], "has_more": False}}
...
def test_include_content_is_explicit(monkeypatch):
    module = _load_module()
    monkeypatch.setenv("LARK_TENANT_ACCESS_TOKEN", "tenant-token-secret")
    monkeypatch.setattr(
        module,
        "_json_request",
        lambda method, url, headers, body=None: {"code": 0, "data": {"items": [_message()], "has_more": False}},
    )
...
def test_empty_window_is_successful_empty_records(monkeypatch):
    module = _load_module()
    monkeypatch.setenv("LARK_TENANT_ACCESS_TOKEN", "tenant-token-secret")
    monkeypatch.setattr(
        module,
        "_json_request",
        lambda method, url, headers, body=None: {"code": 0, "data": {"items": [], "has_more": False}},
    )

    payload = module.fetch_messages(_args())

    assert payload["records"] == []
    assert payload["source"] == "lark-openapi"


def test_api_error_is_clear_and_redacted(monkeypatch, tmp_path, capsys):
    module = _load_module()
    secret = "super-secret-app-secret"
    chat_id = "oc_targetchatsecret1234567890"
    monkeypatch.setenv("LARK_APP_ID", "cli_test")
    monkeypatch.setenv("LARK_APP_SECRET", secret)
    monkeypatch.setenv("LARK_CHAT_ID", chat_id)
    ...
    def fake_request(method, url, headers, body=None):
        ...
        return {
            "code": 230002,
            "msg": f"Bot/User can NOT be out of the chat {chat_id}; secret {secret}",
        }
