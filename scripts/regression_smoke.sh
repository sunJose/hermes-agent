...
# CC闭环 migrated to external plugin at ~/.hermes/plugins/cc-loop/ on 2026-05-25.
# Repo no longer carries the source; verify the plugin is installed, enabled,
# and its 3 CLI commands respond.
CC_LOOP_DIR="$HOME/.hermes/plugins/cc-loop"
if [[ -d "$CC_LOOP_DIR" ]]; then
  ok "plugin dir present: $CC_LOOP_DIR"
else
  fail "plugin dir missing: $CC_LOOP_DIR — restore from ~/.ai-center/plugins-mirror/cc-loop/"
fi

for CMD in rr cc-loop review-gate; do
  if hermes "$CMD" --help >/tmp/cc_help.$$.out 2>&1; then
    ok "hermes $CMD --help responds"
  else
    fail "hermes $CMD --help failed (plugin not enabled? run: hermes plugins enable cc-loop)"
  fi
done
rm -f /tmp/cc_help.$$.out

if [[ -d "$CC_LOOP_DIR/tests" ]]; then
  if python -m pytest "$CC_LOOP_DIR/tests" -q --tb=no >/tmp/cc_pytest.$$.out 2>&1; then
    ok "plugin tests passed ($(grep -oE '[0-9]+ passed' /tmp/cc_pytest.$$.out || echo 'count unknown'))"
  else
    fail "plugin tests failed (see /tmp/cc_pytest.$$.out)"
  fi
else
  warn "no tests dir at $CC_LOOP_DIR/tests"
fi
